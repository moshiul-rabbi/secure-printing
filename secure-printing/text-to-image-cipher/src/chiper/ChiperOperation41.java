 ...
 */
public class ChiperOperation41 {
...
//            int radius = (int)text.charAt(i);
            ...
            double[] xy = generateMinDistanceXY(radius, randomStar, stars, Constants.BOUNDARY_RIGHT, Constants.BOUNDARY_Y_TOP,
                    Constants.BOUNDARY_Y_BOTTOM, Constants.COMPRESSOR);
...
*/
    ...
    public double[] generateMinDistanceXY(int radius, int star, int[][] stars, int boundaryRight, int boundaryTop, int boundaryBottom, int compressor) {
        int angleInDegree = util.generateRandomValue(Constants.MAX_ANGLE_IN_DEGREE, Constants.MIN_ANGLE_IN_DEGREE);
        double[] xy = geometricCalculation.generateXYCompressed2(stars[star][0], stars[star][1], radius, angleInDegree, compressor);
        for(int i=0; i<Constants.NUMBER_OF_STAR; i++) {
            if (i == star)
                continue;

            double distance = geometricCalculation.getDistance(xy[0], xy[1], stars[i][0], stars[i][1]);
//            double radiusCompressed = radius/compressor;

            boolean insideBlackHole = insideBlackHole(xy[0], xy[1]);
            double x_whole = xy[0]+stars[star][0]+Constants.BASE_X;
            double y_whole = xy[1]+stars[star][1]+Constants.BASE_Y;

//            System.out.println("radiusCompressed: " + radiusCompressed + " distance: " + distance);

            if (distance < radius || insideBlackHole || x_whole > boundaryRight ||
                    y_whole < boundaryBottom || y_whole > boundaryTop) {
                return generateMinDistanceXY(radius, star, stars, boundaryRight, boundaryTop, boundaryBottom, compressor);
            }
        }
        return xy;
    }
