 ...
 */
public class ChiperOperation_temp {
    ...
    public double[][] getCordinatesFromText(String text) throws Exception {
        ...
        int[] randomAngles = util.generateRandomValuesDescended(Constants.MIN_ANGLE_IN_DEGREE,Constants.MAX_ANGLE_IN_DEGREE,length);;
...
//        Randomly generating y axis either positve or negative Start
            double value = util.generateRandomValue(0,100);
            int angleInDegree = randomAngles[i];
            if(value%2 == 0)
                angleInDegree = (-1)*angleInDegree;
//        Randomly generating y axis either positve or negative End

            double[] xy = generateMinDistanceXY(radius, randomStar, stars, angleInDegree);
        ...
        // Sorting based on Y
        Arrays.sort(cordinates, new Comparator<double[]>() {
    ...
    public double[] generateMinDistanceXY(int radius, int star, int[][] stars, int angleInDegree) {
        ...
        for(int i=0; i<Constants.NUMBER_OF_STAR; i++) {
            ...
            if (distance < radius || insideBlackHole) {
                ...
                return generateMinDistanceXY(radius, star, stars, angleInDegree);
            }
        }
        return xy;
    }
